  ...
  const loadEvents = async () => {
    try {
        const events = await handledAPIGet(
         userInfo.userType === "seller" ? `/caterers/seller/${userInfo.userId}` : "/caterers"
  ...
  useEffect(() => {
   ...
   return (
     <div className="container mt-5">
       <h1 className="listingHeading__title">Wedding Caterers</h1>
   ...
   {events.map((caterer) => (
    <EventBooking key={caterer} {...caterer} />
  )) }
</div>
)
}
