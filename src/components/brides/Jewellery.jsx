  ...
  const loadEvents = async () => {
    try {
        const events = await handledAPIGet(
         userInfo.userType === "seller" ? `/jewellery/seller/${userInfo.userId}` : "/jewellery"
  ...
  useEffect(() => {
   ...
   return (
     <div className="container mt-5">
       <h1 className="listingHeading__title">Jewellery Stores</h1>
       ...
       {events.map((jewellery) => (
         <EventBooking key={jewellery} {...jewellery} />
       )) }
     </div>
   )
}
